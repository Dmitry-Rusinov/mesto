...
const buttonLike = (evt) => {
  const item = evt.target.closest('.elements__button-like_active');
  item.classList;
}
...
const cardDescription = querySelector.querySelector('#popup__input_card_description');
const picturesLink = querySelector.querySelector('#popup__input_pictures-link');
const newCardDescription = querySelector.querySelector('.elements__title');
const newPicturesLink = querySelector.querySelector('.elements__picture');
const popupAddCard = document.querySelector('#add_card');
const newPopupButtonCloced = popupAddCard.querySelector('.popup__closed');

const form = document.getElementsByName('addCard');
...
newPopupButtonCloced.addEventListener('click', () => togglePopupState(popupAddCard));

function addCardSubmit (evt) {
  evt.preventDefault();  
  changedElements.querySelector('.elements__picture').alt = cardDescription.value;
  changedElements.querySelector('.')
  createCard(item, links);
  togglePopupState(popupAddCard);
}

form.addEventListener('submit', addCardSubmit);
